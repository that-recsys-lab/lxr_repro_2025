...
class LXR_loss(nn.Module):
    ...
    '''
        ...
        user_tensors: tensor representing the users
        i1_tensors:
        i1_id:
        m1: mask for target item
        returns: ???
    '''
    def forward(self, user_tensors, i1_tensors, i1_id, m1):
        ...
        if self.output_type =='single':
            ## Recommender output for the target item by applying m1 mask
        ...
        else: #COMMENT what the 'else' case is (what else can self.output_type be)
            ### target item output
