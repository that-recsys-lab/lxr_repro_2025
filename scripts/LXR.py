...
from scripts.load_data import targ_item, load_data, targ_item_LXR
...
class LXR ():
    ...
    def predict (self):
        ...
        targ_train, targ_test=targ_item_LXR(data_name=self.data_name, recommender_name=self.recommender_name, recommender=self.recommender,kw_dict=self.kw)
        ...
        print(f'======================== LXR ({self.task} task) run for {self.data_name} and {self.recommender_name}========================')
        ...
        for epoch in range(epochs):
            ...
            torch.save(explainer.state_dict(), Path(kw_dict['checkpoints_path'], f'LXR_{self.task}_{self.data_name}_{self.recommender_name}_{epoch}.pt'))
